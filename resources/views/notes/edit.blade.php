    ...
    <div>
        <div class="max-w-4xl mx-auto">
            <div class="bg-white dark:bg-gray-800 rounded-[2rem] card-shadow border border-gray-50 dark:border-gray-700 overflow-hidden">
                <form action="{{ route('notes.update', $note) }}" method="POST" class="p-8 sm:p-12" id="note-form">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-8">
                                    ...
                                    @foreach(['#ffffff', '#fef3c7', '#dbeafe', '#d1fae5', '#fce7f3', '#e0e7ff', '#f3e8ff', '#fee2e2'] as $color)
                                        <label class="relative cursor-pointer group">
                                            <input type="radio" name="color" value="{{ $color }}" class="sr-only peer"
                                                {{ old('color', $note->color) == $color ? 'checked' : '' }}>
                        ...
                        <div class="flex justify-between items-center pt-8">
                            ...
                            <div class="flex space-x-4">
                                <a href="{{ route('notes.show', $note) }}" class="px-8 py-3 bg-gray-50 dark:bg-gray-700 text-gray-600 dark:text-gray-300 font-semibold rounded-xl hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors">
                                    Cancel
                                </a>
