<x-app-layout>
    <x-slot name="header">
        <form action="{{ route('notes.index') }}" method="GET" class="w-full max-w-2xl flex items-center bg-gray-50 dark:bg-gray-800 rounded-full px-6 py-3 border border-gray-100 dark:border-gray-700 focus-within:ring-2 focus-within:ring-yellow-100 dark:focus-within:ring-yellow-900 transition-all">
            ...
            <select name="folder_id" class="bg-transparent border-none focus:ring-0 text-sm text-gray-500 dark:text-gray-400 outline-none ml-4 pl-4 border-l border-gray-200 dark:border-gray-700" onchange="this.form.submit()">
        ...
        @if($notes->count() > 0)
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                @foreach($notes as $note)
                    <div class="relative group rounded-[20px] p-6 card-shadow transition-all duration-300 hover:-translate-y-1 hover:shadow-xl bg-white dark:bg-gray-800 border border-gray-50 dark:border-gray-700" 
                        ...
                        <div class="flex justify-between items-start mb-4">
                            <h3 class="text-lg font-bold text-gray-800 dark:text-gray-100 line-clamp-1 flex-1">
                                <a href="{{ route('notes.show', $note) }}" class="hover:text-yellow-600 dark:hover:text-yellow-400 transition-colors">
                                    {{ $note->title }}
                                </a>
                            </h3>
                            @if($note->is_pinned)
                                <span class="text-pink-500">
                                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path d="M5 5a2 2 0 012-2h6a2 2 0 012 2v2a2 2 0 01-2 2H7a2 2 0 01-2-2V5zm0 8a2 2 0 012-2h6a2 2 0 012 2v2a2 2 0 01-2 2H7a2 2 0 01-2-2v-2z"/></svg>
                                </span>
                            @endif
                        </div>

                        <div class="text-xs text-gray-400 dark:text-gray-500 mb-3">Last edited: {{ $note->updated_at->format('d M Y') }}</div>
                        ...
                        <div class="flex justify-between items-center mt-auto pt-4 border-t border-gray-100/50 dark:border-gray-700/50">
                            ...
                            <div class="flex space-x-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                <form action="{{ route('notes.toggle-pin', $note) }}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit" class="text-gray-400 dark:text-gray-500 hover:text-pink-500 dark:hover:text-pink-400 transition">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 23V5z"></path></svg>
                                    </button>
                                </form>
                                <a href="{{ route('notes.edit', $note) }}" class="text-gray-400 dark:text-gray-500 hover:text-yellow-500 dark:hover:text-yellow-400 transition">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                </a>
                                <button onclick="confirmDelete({{ $note->id }})" class="text-gray-400 dark:text-gray-500 hover:text-red-500 dark:hover:text-red-400 transition">
